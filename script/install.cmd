...
param(
    ...
    [string]$ServiceName = "xlsdwanService",
    [Parameter(Mandatory = $false)]
    [switch]$Uninstall
)
...
function Get-BasicNetworkConfig {
    ...
    if (Show-YesNoPrompt -Message "是否使用公共共享节点来发现对等节点？") {
        $options += "--external-node $(Get-InputWithDefault -Prompt "公共节点地址(格式:协议://IP:端口)" -DefaultValue "tcp://public.xlsdwan.cn:11010")"
    }
    if (Show-YesNoPrompt -Message "是否添加对等节点？") {
        $peers = @()
        do {
            $peers += Get-InputWithDefault -Prompt "对等节点地址" -DefaultValue "tcp://public.xlsdwan.cn:11010"
    ...
    $ipChoice = Show-MultipleChoicePrompt -Message "请选择IP分配方式" `
        ...
        -Helps @("自定义此节点的IPv4地址，如果为空则仅转发数据包", "由xlsdwan自动确定并设置IP地址", "将仅转发数据包，不会创建TUN设备") `
        -DefaultIndex 1
    switch ($ipChoice) {
...
function Get-ExtraAdvancedOptions {
    ...
    if (Show-YesNoPrompt -Message "是否配置日志选项？" -DefaultIndex 1) {
        ...
        if (Show-YesNoPrompt -Message "是否指定日志目录？" -DefaultIndex 1) {
            $logDir = Get-InputWithDefault -Prompt "日志目录路径" -DefaultValue "$env:ProgramData\xlsdwan\logs"
...
$host.ui.rawui.WindowTitle = if ($Uninstall) { "卸载xlsdwan服务" } else { "安装xlsdwan服务" }  
Clear-Host
$ScriptRoot = (Get-Location).Path
$ServicesPath = Join-Path $ScriptRoot "services"
$RequiredFiles = @("xlsdwan-core.exe", "xlsdwan-cli.exe", "nssm.exe", "Packet.dll", "wintun.dll")
...
try {
    ...
    if ($Uninstall) {
        ...
        if (Show-YesNoPrompt -Message "确认安装配置？" -DefaultIndex 1) {
            & $nssm install $ServiceName (Join-Path $ScriptRoot "xlsdwan-core.exe")
            & $nssm set $ServiceName AppParameters $arguments
            & $nssm set $ServiceName Description "xlsdwan 核心服务"
