...
#[derive(Parser, Debug)]
#[command(name = "xlsdwan-core", author, version = XLSDWAN_VERSION , about, long_about = None)]
struct Cli {
    #[arg(
        short = 'w',
        long,
...
async fn run_main(cli: Cli) -> anyhow::Result<()> {
    ...
    if cli.config_server.is_some() {
        ...
        println!("Official config website: https://sdwan.xiaolin.cc/web");
    ...
    if let Some(config_files) = cli.config_file {
        ...
        for config_file in config_files {
            ...
            println!(
                "Starting xlsdwan from config file {:?} with config:",
                config_file
            );
            println!("############### TOML ###############\n");
            println!("{}", cfg.dump());
    ...
    if crate_cli_network {
        ...
        println!("Starting xlsdwan from cli with config:");
        println!("############### TOML ###############\n");
        println!("{}", cfg.dump());
        println!("-----------------------------------");
...
async fn main() -> ExitCode {
    ...
    if let Some(shell) = cli.gen_autocomplete {
        ...
        easytier::print_completions(shell, &mut cmd, "xlsdwan-core");
