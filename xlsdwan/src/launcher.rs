...
pub struct XlsdwanLauncher {
    ...
    data: Arc<XlsdwanData>,
}

impl XlsdwanLauncher {
    pub fn new(fetch_node_info: bool) -> Self {
        ...
        Self {
            ...
            data: Arc::new(XlsdwanData::default()),
        }
    }

    async fn handle_xlsdwan_event(event: GlobalCtxEvent, data: &XlsdwanData) {
        ...
        tasks.spawn(async move {
            ...
            loop {
                match receiver.recv().await {
                    Ok(event) => {
                        Self::handle_xlsdwan_event(event.clone(), &data_c).await;
                    }
                ...
                loop {
                    ...
                    *data_c.tun_dev_name.write().unwrap() =
                    ...
                    let node_info = MyNodeInfo {
                        ...
                        version: XLSDWAN_VERSION.to_string(),
        ...
        F: FnOnce() -> Result<TomlConfigLoader, anyhow::Error> + Send + Sync,
    {
        ...
        self.thread_handle = Some(std::thread::spawn(move || {
            ...
            let ret = rt.block_on(Self::xlsdwan_routine(
                cfg,
                stop_notifier.clone(),
                data,
                fetch_node_info,
...
pub struct NetworkInstance {
    ...
    launcher: Option<XlsdwanLauncher>,
...
impl NetworkInstance {
    ...
    pub fn is_xlsdwan_running(&self) -> bool {
    ...
    pub fn start(&mut self) -> Result<EventBusSubscriber, anyhow::Error> {
        if self.is_xlsdwan_running() {
            return Ok(self.subscribe_event().unwrap());
        }

        let launcher = XlsdwanLauncher::new(self.get_fetch_node_info());
