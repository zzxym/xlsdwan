...
#[derive(Parser, Debug)]
#[command(name = "xlsdwan-cli", author, version = XLSDWAN_VERSION, about, long_about = None)]
struct Cli {
    /// the instance name
...
enum SubCommand {
    ...
    VpnPortal,
    #[command(about = "inspect self xlsdwan-core status")]
    Node(NodeArgs),
    #[command(about = "manage xlsdwan-core as a system service")]
    Service(ServiceArgs),
...
enum ServiceSubCommand {
    #[command(about = "register xlsdwan-core as a system service")]
    Install(InstallArgs),
    #[command(about = "unregister xlsdwan-core system service")]
    Uninstall,
    #[command(about = "check xlsdwan-core system service status")]
    Status,
    #[command(about = "start xlsdwan-core system service")]
    Start,
    #[command(about = "stop xlsdwan-core system service")]
    Stop,
}
...
struct InstallArgs {
    ...
    #[arg(long, help = "path to xlsdwan-core binary")]
    ...
    #[arg(
        ...
        help = "args to pass to xlsdwan-core"
    )]
        ...
        impl From<PeerRoutePair> for PeerTableItem {
            fn from(p: PeerRoutePair) -> Self {
                ...
                PeerTableItem {
                    ...
                    ipv4: route
                        .ipv4_addr
                        .map(|ip: xlsdwan::proto::common::Ipv4Inet| ip.address.unwrap_or_default())
                        .map(|ip| ip.to_string())
                        .unwrap_or_default(),
...
"#,
            ...
            match service_args.sub_command {
                ServiceSubCommand::Install(install_args) => {
                    ...
                    let bin_path = std::fs::canonicalize(bin_path).map_err(|e| {
                        anyhow::anyhow!("failed to get xlsdwan core application: {}", e)
                    })?;
        ...
        SubCommand::GenAutocomplete { shell } => {
            ...
            xlsdwan::print_completions(shell, &mut cmd, "xlsdwan-cli");
        }
    }
...
mod win_service_manager {
    ...
    use xlsdwan::common::constants::WIN_SERVICE_WORK_DIR_REG_KEY;
