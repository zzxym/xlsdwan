...
async fn respond_stun_packet(
    ...
    // we only respond xlsdwan stun req, whose tid has 0xdeadbeef prefix
    if tid.as_bytes()[0..4] != [0xde, 0xad, 0xbe, 0xef] {
        anyhow::bail!("stun req tid not from xlsdwan");
    }
...
mod tests {
    ...
    async fn ipv6_domain_pingpong() {
        ...
        let mut connector =
            UdpTunnelConnector::new("udp://sdwan.xiaolin.cc:31016".parse().unwrap());
